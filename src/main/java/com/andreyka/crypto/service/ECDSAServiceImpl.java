...
@Service
public class ECDSAServiceImpl implements ECDSAService {
    private final KeyPairService keyPairService;
    private final ECPointMathService ecPointMathService;

    @Autowired
    public ECDSAServiceImpl(KeyPairService keyPairService, ECPointMathService ecPointMathService) {
        this.keyPairService = keyPairService;
        this.ecPointMathService = ecPointMathService;
    }

    @Override
    public ECPoint getSignature(String hash, BigInteger yourPrivateKey) throws CloneNotSupportedException {
        BigInteger r;
        KeyPair keyPair;
        BigInteger s;
        ...
        do {
            do {
                keyPair = this.keyPairService.generateKeyPair();
    ...
    @Override
    public boolean isValid(String hash, ECPoint otherPublicKey, ECPoint signature) throws CloneNotSupportedException {
        ...
        ECPoint first_summary = this.ecPointMathService.multiply(new ECPoint(Inputs.values()), u1);
        ECPoint second_summary = this.ecPointMathService.multiply(otherPublicKey, u2);

        ECPoint pointP = this.ecPointMathService.add(first_summary, second_summary);
