...
import com.andreyka.server.handlers.RspHandler;
...
public class NioClient implements Runnable {
    ...
    public static void main(String[] args) {
        try {
            ...
            client.send("HIHIHI".getBytes(), handler);
        ...
        while (true) {
            try {
                ...
                synchronized (this.pendingChanges) {
                    for (ChangeRequest pendingChange : this.pendingChanges) {
                        ChangeRequest change = pendingChange;
                        switch (change.type) {
                            case ChangeRequest.CHANGEOPS -> {
                                SelectionKey key = change.socket.keyFor(this.selector);
                                key.interestOps(change.ops);
                            }
                            case ChangeRequest.REGISTER -> change.socket.register(this.selector, change.ops);
                        }
                    }
